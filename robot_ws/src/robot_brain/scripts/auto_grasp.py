...
def main():
    ...
    while True:
        ...
        cv2.putText(img, f"FPS:{fps:.1f} YOLOv8n", (5,20), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0,255,0), 2)
        if closest:
            cv2.putText(img, f"[{closest[0]} {closest[3]*100:.0f}cm] G=grasp H=home Q=quit",
                       (5,h-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0), 2)
        cv2.imshow("Grasp", img)
        key = cv2.waitKey(1)&0xFF

        if key == ord('q'): break
        elif key == ord('h'): arm.send(HOME); time.sleep(1.5)
        elif key == ord('g') and closest:
            _, cx, _, dist = closest
            j1 = max(-1.2, min(1.2, (cx-w/2)/w*0.6))
            print(f"[Grasp] {closest[0]} {dist*100:.0f}cm j1={j1:.2f}")
            ...
            arm.send(PLACE); time.sleep(2)
            arm.send(RELEASE); time.sleep(1)
            arm.send(HOME)
